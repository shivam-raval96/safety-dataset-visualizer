...
Paper, dataset, organism, and distilled-model cards let visitors enter a name and choose **Add comment**. No account is needed. Comments appear as soon as the database confirms the save and are fetched again on reload. Names are self-reported and comments are public. Failed submissions preserve the draft; retries use the same ID to avoid duplicates.

The standalone comment Worker is deployed through the existing Sites project, with D1 persistence. The atlas remains on GitHub Pages. `data/comments-service.json` identifies the live API. `npm run build:comments` builds only the service and its migrations; `npm run build:static` builds GitHub Pages. The catalog at `public/comment-targets.json` is regenerated during builds so the service can validate newly added cards without a backend deployment.

The **Back up direct comments** workflow runs hourly and on demand. It downloads all comment pages, merges them into `data/comments.md` without duplicating IDs or removing existing entries, commits the backup, and republishes Pages. GitHub may delay scheduled runs; a successful save does not wait for this backup. The database is authoritative for direct comments. Existing GitHub-issue comments remain visible through the Markdown fallback, and the old issue workflow remains available for those submissions.

Card identity includes type, title, and URL to distinguish organisms sharing a research link. Preserve or migrate those fields when renaming a card. Database migrations live in `drizzle/`; applied migrations must remain unchanged. Maintainer changes to direct comments belong in the database and should be reflected in the backup too.

Run `npm run build:comments` followed by `node --test tests/comments.test.mjs tests/comments-service.test.mjs` with Node 22 to check validation, persistence, retries, concurrent submissions, rate limits, pagination, and Markdown backup merging. No production comment is created by these tests.

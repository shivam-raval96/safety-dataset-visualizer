...
def relevant(topic: str, title: str, text: str, keyword: str) -> bool:
    combined = f"{title} {text}"
    return bool(AI_SIGNAL.search(combined) and SAFETY_SIGNAL.search(combined) and TOPIC_REQUIRED[topic].search(combined) and keyword.casefold() in title.casefold())
...
def arxiv_results(topic: str, keywords: list[str], limit: int) -> list[dict[str, Any]]:
    ...
    for entry in root.findall("atom:entry", ATOM):
        ...
        if not any(relevant(topic, title, summary, keyword) for keyword in keywords):
            continue
...
def lesswrong_results(topic: str, keyword: str, posts: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for post in posts:
        if "_atlas_search" not in post:
            ...
            post["_atlas_search"] = (title, body, f"{title} {tags} {body[:3000]}")
        title, body, searchable = post["_atlas_search"]
        needle = keyword.casefold()
        if needle not in title.casefold() or not AI_SIGNAL.search(searchable) or not SAFETY_SIGNAL.search(searchable) or not TOPIC_REQUIRED[topic].search(searchable):
            continue
...
</style></head><body><header><h1>Paper Atlas candidate review</h1><p>Generated {generated_at} · selections are saved in this browser</p><div class="tools"><input id="search" type="search" placeholder="Search title, author, summary…"><select id="topicFilter"><option value="">All topics</option>{topic_options}</select><select id="kindFilter"><option value="">Papers + LessWrong</option><option>Paper</option><option>LessWrong</option></select><button id="approveVisible">Approve visible</button><button id="clear">Clear tags</button><button class="primary" id="export">Export approved</button></div></header><main><div class="summary" id="summary"></div><div class="grid" id="grid"></div></main>
...
def main() -> int:
    ...
    if args.html_output:
        write_review_html(args.html_output, additions, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC"))

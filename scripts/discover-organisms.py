...
def hf_targets(html: str) -> list[tuple[str, str]]:
    ...
    for value in parser.items:
        ...
        if not parts or parts[0] in {"datasets", "spaces", "papers", "collections", "docs"}:
            continue
        if len(parts) == 1:
...
def models_for_target(kind: str, value: str, per_author: int) -> list[dict[str, Any]]:
    ...
    return [item for model in results if (item := model_metadata(model["id"]))]
    ...
    for post in posts:
        ...
        for target in hf_targets(html):
            for model in models_for_target(*target, per_author):
                haystack = f"{model['id']} {' '.join(model['tags'])} {model['description']} {text[:4000]}"
                if TERMS.search(haystack) and TRAINING.search(haystack):
...
def main() -> int:
    ...
    entries = ai_entries(candidates, api_key, args.model) if candidates and not args.no_ai else [heuristic_entry(item, index) for index, item in enumerate(candidates)]

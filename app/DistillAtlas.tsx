...
export default function DistillAtlas({ onBack, onSwitch }: { onBack: () => void; onSwitch: () => void }) {
  ...
  const draw = useCallback(() => {
    ...
    for (const model of visible) {
      ...
      context.fillStyle = pipelineColor(model.pipeline); context.globalAlpha = model === selected ? 1 : .62; context.fill();
      if (model === selected) { context.strokeStyle = "#17211d"; context.lineWidth = 2; context.globalAlpha = 1; context.stroke(); }
    }
    context.globalAlpha = 1;
  }, [pointPosition, selected, visible, view.scale]);
  ...
  const pointerMove = (event: PointerEvent<HTMLDivElement>) => { const drag = dragRef.current; if (!drag) return; const dx = event.clientX - drag.x, dy = event.clientY - drag.y; if (Math.abs(dx) + Math.abs(dy) > 3) drag.moved = true; setView((current) => ({ ...current, x: drag.ox + dx, y: drag.oy + dy })); };
  ...
  return <main className="app-shell organism-shell distill-shell">
    ...
    <section className="workspace">
      ...
      <section className="map distill-map" aria-label="UMAP of distilled Hugging Face models">
        ...
        <div className="distill-plot" ref={plotRef} onWheel={wheel} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={() => { dragRef.current = null; }}><canvas ref={canvasRef}/>{!visible.length && <div className="empty">No models match this view.<button onClick={() => {setQuery("");setPipeline("All tasks");}}>Show all models</button></div>}</div>

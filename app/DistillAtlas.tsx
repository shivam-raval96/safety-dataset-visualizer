...
function modelSizeBillions(model: DistillModel) {
  const signals = `${model.id} ${model.baseModel}`;
  const sizes = [...signals.matchAll(/(?:^|[-_/\s])(\d+(?:\.\d+)?)\s*([bm])(?:[-_.\s]|$)/gi)]
    .map((match) => Number(match[1]) * (match[2].toLowerCase() === "m" ? .001 : 1))
    .filter(Number.isFinite);
  return sizes.length ? Math.max(...sizes) : null;
}

function modelSizeLabel(model: DistillModel) {
  const size = modelSizeBillions(model);
  if (size === null) return "—";
  return size < 1 ? `${Math.round(size * 1000)}M` : `${Number(size.toFixed(2))}B`;
}
...
export default function DistillAtlas({ onBack, onSwitch }: { onBack: () => void; onSwitch: () => void }) {
  ...
  const listGroups = useMemo(() => categories.map(([family]) => ({
    family,
    models: visible
      .filter((model) => categoryFor(model) === family)
      .sort((a, b) => (modelSizeBillions(b) ?? -1) - (modelSizeBillions(a) ?? -1) || b.downloads - a.downloads),
  })).filter((group) => group.models.length), [categories, visible]);
  ...
  useEffect(() => { draw(); const observer = new ResizeObserver(draw); if (plotRef.current) observer.observe(plotRef.current); return () => observer.disconnect(); }, [display, draw]);
  ...
  return <main className="app-shell organism-shell distill-shell">
    ...
    <section className="workspace">
      ...
      <aside className="detail"><div className="organism-panel" key={selected.id}><div className="detail-top"><div className="dataset-icon organism-icon" style={{background:categoryColor(categoryFor(selected))}}>HF</div></div><p className="detail-category"><span style={{background:categoryColor(categoryFor(selected))}}/>{categoryFor(selected)}</p><h2>{selected.id.split("/").at(-1)}</h2><p className="org">by {selected.id.split("/")[0]} · {selected.pipeline}</p><p className="description">{selected.baseModel ? `A distilled model derived from ${selected.baseModel}.` : "A public Hugging Face model matched by the distill search and positioned from its model-card metadata."}</p><div className="stats"><div><span>Downloads</span><strong>{compactNumber(selected.downloads)}</strong></div><div><span>Likes</span><strong>{compactNumber(selected.likes)}</strong></div><div><span>Library</span><strong>{selected.library || "Not specified"}</strong></div><div><span>License</span><strong>{selected.license || "Not specified"}</strong></div></div>{selected.tags.length > 0 && <div className="detail-section"><p className="eyebrow">Model card signals</p><div className="tags">{selected.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></div>}<a className="open-button" href={`https://huggingface.co/${selected.id}`} target="_blank" rel="noreferrer">Open model card <span>↗</span></a><Comments kind="model" title={selected.id} url={`https://huggingface.co/${selected.id}`}/></div></aside>
    </section>
  </main>;
}
